...
**convoy-or** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `convoy-or` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
convoy-or ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
convoy-or ... --format json | cognis-connect emit --to slack --url $URL --token $TOK   # Slack
convoy-or ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `convoy-or` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="convoy-or") for rec in records]   # records = your JSON output
