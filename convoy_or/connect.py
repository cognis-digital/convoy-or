"""Native cognis-connect emit for convoy-or — forward findings to any platform.

Maps convoy-or's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    convoy-or ... --format json | convoy-or-emit --to stix
    convoy-or-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('finding', '').split(':')[0].strip()
        out['severity'] = rec.get('severity', 'info')
        out['type'] = rec.get('type', 'finding')
        out['description'] = rec.get('finding', '').split(':', 1)[1].strip() if ':' in rec.get('finding', '') else ''
